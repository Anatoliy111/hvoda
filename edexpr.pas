...
begin
  ...
  begin
    if MethodName='RECNO' then
      ...
      raise Exception.Create('Попытка вызвать неизвестную функцию '+MethodName);
  end
  else
  begin
    ...
    begin
      ...
      begin
        ...
        raise Exception.Create('Попытка открыть неизвестную таблицу '+UpperCase(Params[0]));
    end
...
// получить среднее число людей за месяц, проживающих по данному счету
var val:variant;
begin
  result:=0;
